...
export class BoardSide {
    ...
    private initializePegHoles(side: any): void{
        ...
        var distanceBetween = this.boardLength / 17;
        var currentx = this.startCoordinates.x;
        var currenty = this.startCoordinates.y;
        side.mainTrackPositions.forEach(trackPosition => {
            ...
            currentx = currentx + this.displacedX(distanceBetween, this.angle + this.LEFT);//currentx - distanceBetween;
        ...
        side.startPositions.forEach(startPosition => {
            if (counter == 3){
                this.pegHoles.set(startPosition.id, new PegHole(this.pegHoles.get(this.color + 'Start-2').x + this.displacedX(distanceBetween, this.angle + this.LEFT), this.pegHoles.get(this.color + 'Start-2').y - this.displacedY(distanceBetween, this.angle + this.LEFT), startPosition.pegColor, startPosition.hasPeg));
            }
            else if (counter == 4){
                this.pegHoles.set(startPosition.id, new PegHole(this.pegHoles.get(this.color + 'Start-2').x + this.displacedX(distanceBetween, this.angle), this.pegHoles.get(this.color + 'Start-2').y - this.displacedY(distanceBetween, this.angle), startPosition.pegColor, startPosition.hasPeg));
            }
             else{
        ...
        side.homePositions.forEach(homePosition => {
            if (counter < 2){
                this.pegHoles.set(homePosition.id, new PegHole(currentx + this.displacedX(distanceBetween, this.angle + this.UP), currenty - this.displacedY(distanceBetween, this.angle + this.UP), homePosition.pegColor, homePosition.hasPeg));
                currentx = currentx + this.displacedX(distanceBetween, this.angle + this.UP);
                currenty = currenty - this.displacedY(distanceBetween, this.angle + this.UP);
            }
            else if (counter >= 2 && counter < 4){
                this.pegHoles.set(homePosition.id, new PegHole(currentx + this.displacedX(distanceBetween, this.angle + this.LEFT), currenty - this.displacedY(distanceBetween, this.angle + this.LEFT), homePosition.pegColor, homePosition.hasPeg));
